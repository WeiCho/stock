...
└── web/                   # React 19 + Vite + TS strict + Tailwind + lightweight-charts v5 + react-i18next
    ...
    │   ├── i18n/          # index.ts（react-i18next init + toggleLang）+ zh.json / en.json（各 332 keys）
...
### i18n（react-i18next）
- 兩本字典 `web/src/i18n/zh.json`（zh-TW，預設）+ `en.json`，各 332 keys（zh/en parity）
...
- [x] Phase 10：美股支援（Yahoo → 共用 daily_price）+ 6 大整合（crypto / FX / movers / SEC Form 4 / Finnhub / TXO PCR）+ 進階台股基本面 + 觀察清單（2 新表）+ 比較視圖 + 全面 i18n（zh/en 332 keys）+ .env 自動載入（python-dotenv）+ pytest 52 + 48 routes
- [x] Phase 11：Fugle 個股/ETF 即時報價（含五檔，每 3 秒輪詢 + 交易時段 gate）+ SEC User-Agent 改 env + market_movers 去重 + pytest 55 + 49 routes
